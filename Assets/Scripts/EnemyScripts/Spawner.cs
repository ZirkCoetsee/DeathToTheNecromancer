...
//        NextWave();
    ...
    private void Update()
    {
        if (enemiesRemaingToSpawn > 0 && Time.time > nextSpawnTime)
        {
            ...
            EnemyTesting spawnedEnemy = Instantiate(enemy, new Vector3(this.transform.position.x,0f,this.transform.position.z) ,Quaternion.identity) as EnemyTesting;
            spawnedEnemy.OnDeath += OnEnmyDeath;
        }
    }
